...
public class StoreResource {
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Store is created Successfully"),
	...
	@PostMapping("/stores")
	public Store addStore(@RequestHeader (value="userId" , required = true) Long userId ,@Valid @RequestBody Store store) {
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Store is retrieved Successfully"),
            @ApiResponse(code = 400, message = "Input JSON is wrong"),
            @ApiResponse(code =401 , message = "User is Not authorized for this operation"),
            @ApiResponse(code = 412, message = "Validation failed Store Data."),
            @ApiResponse(code = 500, message = "Internal server error")
	...
	@ApiResponses(value = {
            @ApiResponse(code = 200, message = "Store is retrieved Successfully by storeId"),
            @ApiResponse(code = 400, message = "Input JSON is wrong"),
            @ApiResponse(code =401 , message = "User is Not authorized for this operation"),
            @ApiResponse(code = 412, message = "Validation failed Store Data."),
            @ApiResponse(code = 500, message = "Internal server error")
    })
	@GetMapping("/stores/id/{storeId}")
	...
	@ApiResponses(value = {
            @ApiResponse(code = 200, message = "Store is retrieved Successfully by storeName"),
            @ApiResponse(code = 400, message = "Input JSON is wrong"),
            @ApiResponse(code =401 , message = "User is Not authorized for this operation"),
            @ApiResponse(code = 412, message = "Validation failed Store Data."),
            @ApiResponse(code = 500, message = "Internal server error")
	...
	public Optional<Store> getStoreByName(@PathVariable ("storeName") @NotNull String storeName) {
	...
	@DeleteMapping("/stores/id/{storeId}")
	public ResponseEntity<Void> deleteStoreById(@RequestHeader (value="userId" , required=true) Long userId ,@PathVariable ("storeId") Long storeId) {
		if(userId!=null && userService.isUserExist(userId)) {		
			if(userService.getUserRole(userId) == RoleConstant.RETAIL_ADMIN || userService.isStoreAdmin(userId,storeId)) {				
	...
	@DeleteMapping("/stores/name/{storeName}")
	public ResponseEntity<Void> deleteStoreByName(@RequestHeader (value="userId" , required=true) Long userId ,@PathVariable ("storeName") String storeName) {
		if(userId!=null && userService.isUserExist(userId)) {		
			//if(userService.getUserRole(userId) == RoleConstant.RETAIL_ADMIN || userService.isStoreAdmin(userId,storeId)) {				
	...
	@ApiResponses(value = {
            @ApiResponse(code = 200, message = "Product is added in store Successfully"),
            @ApiResponse(code = 400, message = "Input JSON is wrong"),
            @ApiResponse(code =401 , message = "User is Not authorized for this operation"),
	...
	public StoreProductsDTO addStoreProducts(@RequestHeader (value = "userId", required = true) Long userId ,@Valid @RequestBody StoreProductsDTO storeProducts) {
		...
		throw new StoreNotFoundException("Store not exist");
	}

	@ApiOperation(value = "Get list of Products in Store by StoreId")
	@ApiResponses(value = {
            @ApiResponse(code = 200, message = "Products retrieved Successfully of paticular store"),
            @ApiResponse(code = 400, message = "Input JSON is wrong"),
            @ApiResponse(code =401 , message = "User is Not authorized for this operation"),
            @ApiResponse(code = 412, message = "Validation failed Store Data."),
            @ApiResponse(code = 500, message = "Internal server error")
	...
	public List<StoreProductBean> getStoreProducts( @PathVariable ("storeId") @NotNull Long storeId) {
		if(storeId !=null && storeService.isStoreExist(storeId)) {
			return storeProductService.getStoreProducts(storeId);
		}
		throw new StoreNotFoundException("Store is Not Exist");
	}
	...
	public StoreProductBean getStoreProductById(@PathVariable ("storeId") Long storeId, @PathVariable ("productId") Long productId) {
		if(storeId != null && storeService.isStoreExist(storeId)) {
